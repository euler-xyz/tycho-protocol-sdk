...
interface IEulerSwapFactory {
    ...
    /// @dev The pool address is deterministically generated using CREATE2 with
    /// a salt derived from
    ///      the euler account address and provided salt parameter. This allows
    /// the pool address to be
    ///      predicted before deployment.
    /// @param params Core pool parameters including vaults, account, fees, and
    /// curve shape
    ...
    /// @dev This function removes the pool from the factory's tracking and
    /// emits a PoolUninstalled event
    /// @dev The function can only be called by the Euler account that owns the
    /// pool
    /// @dev If no pool is installed for the caller, the function returns
    /// without any action
    function uninstallPool() external;

    /// @notice Compute the address of a new EulerSwap pool with the given
    /// parameters
    /// @dev The pool address is deterministically generated using CREATE2 with
    /// a salt derived from
    ///      the euler account address and provided salt parameter. This allows
    /// the pool address to be
    ///      predicted before deployment.
    /// @param poolParams Core pool parameters including vaults, account, and
    /// fee settings
    /// @param salt Unique value to generate deterministic pool address
    /// @return Address of the newly deployed pool
    ...
    function poolsSlice(uint256 start, uint256 end)
        external
        view
    ...
    function poolsByPairLength(address asset0, address asset1)
        external
        view
        returns (uint256);

    /// @notice Returns a slice of pools for a specific asset pair
    /// @dev Returns a subset of the pools array for the given asset pair from
    /// start to end index
    /// @param asset0 The address of the first asset
    /// @param asset1 The address of the second asset
    /// @param start The starting index of the slice (inclusive)
    /// @param end The ending index of the slice (exclusive)
    /// @return An array containing the requested slice of pool addresses for
    /// the asset pair
    ...
    /// @dev Returns the complete array of pool addresses for the given asset
    /// pair
    /// @param asset0 The address of the first asset
    /// @param asset1 The address of the second asset
    /// @return An array containing all pool addresses for the specified asset
    /// pair
    function poolsByPair(address asset0, address asset1)
        external
        view
        returns (address[] memory);

    /// @notice Returns the pool address associated with a specific holder
    /// @dev Returns the pool address from the EulerAccountState mapping for the
    /// given holder
